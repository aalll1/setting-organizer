...
- 新增 `stateArchive.js`，集中处理状态条目归档标记。
- 新增 `stateMerger.js`，实现人物、势力、任务、道具的确定性合并和 diff 生成。
- 新增 `stateMerger.test.mjs`，覆盖人物地点变化归档、道具持有人变化 diff、重复导入不重复。
- 更新 `docs/DATA_MODEL.md` 和 `docs/TESTING.md`，记录合并身份键、归档字段和测试范围。
- 本任务只实现 core 层，不写 localStorage，不写 SillyTavern。
...
- 覆盖同名人物多个所在地、同名人物多个状态、同一任务多个状态、同一道具多个持有人、同一势力多个态度。
- 覆盖历史归档条目仍标记为 active 的边界冲突。
- 新增 `conflictDetector.test.mjs`，确认检测过程不修改输入状态。
- 当前只返回冲突提示，不调用模型，不自动修正。
...
- 新增 `conflictPanel.js`，按严重程度展示冲突对象、字段、取值、来源 ID、来源范围和处理建议。
- `statePanel.js` 增加显式“检测状态冲突”操作；检测结果不会阻塞状态草稿浏览或编辑。
- `conflictDetector.js` 在只读结果中返回去重后的 `sourceMessageRanges`。
- 测试覆盖冲突范围、空结果与状态面板渲染。
...
- 新增 `worldbookSyncBuilder.js`，将已校验的 `CampaignState` 转换为现有 `LorebookDraft` 兼容结构。
- 草稿分为永久设定、当前状态、任务、人物、势力、关键道具和历史归档；归档草稿默认禁用。
- builder 返回独立的新增、修改、未变化、移除预览，不读取或覆盖现有酒馆世界书。
- adapter 仅保留状态来源元数据到 World Info extensions，不引入写入决策。
...
- 状态面板提供世界书草稿预览、分类选择和显式“创建新世界书”操作。
- 状态同步复用既有备份、创建、旧数据校验和原生世界书交接报告。
- 实测创建 `设定整理器导入 20260710103723705`，旧世界书校验通过；不覆盖或删除已有世界书。
...
- 新增固定内置模板：通用、历史模拟、跑团 / DND。
- 模板定义集中管理 prompt 侧重点与状态 UI 分组；设置层只保存模板 ID。
- 默认通用模板保留既有基础状态字段和 UI 分组，切换不会修改已有草稿。
...
- 新增通用、历史模拟、跑团 / DND 三组输入与期望草稿样例。
- `templateSamples.test.mjs` 使用 mock 状态分析自动验证样例结构、地点前缀和模板 prompt 标识。
- 新增 `docs/USER_GUIDE.md`，说明适用场景、使用方式与限制。
...
- 新增 `src/constants/`，集中聊天范围、默认 confidence、日志文本阈值和关键日志事件名。
- `chatAdapter` 保持 `CHAT_RANGES` 的兼容导出，避免 UI 和测试导入回归。
- 内部使用 `lorebook`，仅 adapter 使用 `worldInfo`；用户可见中文“世界书”未改动。
...
- 新增零依赖 `package.json`、`npm run check` 和 `npm test`。
- `check` 递归检查扩展 JavaScript 语法；`test` 运行所有 `*.test.mjs`。
- `cdp-check.mjs` 明确作为需要 websocket URL 的运行时辅助脚本，不进入默认单元测试。
