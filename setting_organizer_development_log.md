...
- 确认 GitHub `master` 为开发分支，而 SillyTavern 原生安装实际使用仓库 `main` 分支。
- 原生安装分支此前仍停留在 `0.2.1`，因此扩展管理器更新不会取得 `0.7.1` 的高对比度样式与后续功能。
- 发布副本 README、功能说明和原生安装说明更新为 `0.7.1`，包含剧情状态、内置模板、规则级冲突提示、状态世界书预览及 PC/移动端高对比度表单说明。
- 发布时保持 `main` 根目录为可直接安装的扩展目录；开发文档、测试记录和运行时目录仍只保留在 `master`。

## 2026-07-11

### 移动端表单可读性修复

- 根据 MuMu 截图反馈，修复浅色输入框、下拉选项和禁用操作按钮继承酒馆主题后文字对比度不足的问题。
- `setting-organizer/style.css` 与 `setting-organizer-native-install/style.css` 统一设置表单前景色、白色控件背景、边框、占位文字和原生 `option` 颜色。
- 禁用按钮不再通过低 opacity 表示不可操作，改用可读的深灰文字与浅灰背景；`cursor: wait` 仍保留，状态语义不变。
- 影响范围仅为扩展视觉样式，不修改分析、导入、日志或数据存储逻辑。
- 本地 `npm.cmd run check` 已检查 43 个 JavaScript 文件，`npm.cmd test` 的 29 组测试均通过；本地酒馆运行时未安装扩展，无法在该页面执行最终 CSS 截图验证。

### 问题复盘

- 原因：插件缺少控件颜色兜底，Android 浏览器会把酒馆主题的浅色前景继承到白色原生表单控件。
- 预防：插件使用浅色原生输入控件时，必须同时声明前景色、背景色、占位符和 `option` 的颜色，不能依赖主题默认值。
...
## 2026-07-10 TC-37 世界书同步确认流程

- 完成 `TC-37`。
- 状态面板增加世界书预览、分类选择和显式创建新世界书操作；写入复用 `importLorebookDraft()` 的备份、创建和旧数据校验。
- 创建报告修正世界书/角色名称的类型化展示。
- SillyTavern 1.18.0 实测：mock 状态分析、预览与创建均通过；测试世界书为 `设定整理器导入 20260710103723705`，备份为 `backup_20260710103723706_u6pp7f`。
- 不覆盖、删除或自动禁用既有世界书，不提供后台持续同步。
- 全量回归：39 个 JavaScript 文件语法检查通过，26 个无参数 `.mjs` 测试通过。

## 2026-07-10 TC-38 模板定义与选择

- 完成 `TC-38`。
- 新增 `stateTemplates.js`，集中定义通用、历史模拟、跑团 / DND 的模板 ID、提示词重点与 UI 分组。
...
## 2026-07-10 TC-40 正式 docs 目录拆分

- 完成 `TC-40`。
- 新增职责明确的路线图、开发规范、架构、兼容性、变更、已知问题和 Agent 执行指南。
- README 保留安装和快速使用入口，增加正式 docs 索引；历史根目录规划、日志和报告不删除。
- 未改业务代码；10 份正式文档和 README 索引校验通过，全量回归通过：40 个 JavaScript 文件语法检查，28 个无参数 `.mjs` 测试。

## 2026-07-11 TC-41 常量与命名边界整理
