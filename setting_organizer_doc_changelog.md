...
### 2026-07-10：完成 TC-40 正式 docs 目录拆分

变更类型：文档 / 工程化

主要变化：

- 新建正式 docs 维护目录文件，按路线、开发、架构、测试、兼容性、变更与问题拆分职责。
- README 增加文档入口，旧根目录材料保留为历史索引。

验证情况：

- 10 份正式文档和 README 索引校验通过；全量回归通过：40 个 JavaScript 文件语法检查，28 个无参数 `.mjs` 测试。

### 2026-07-11：完成 TC-41 常量与命名边界整理

变更类型：重构 / 测试 / 文档

主要变化：

- 集中聊天范围、默认 confidence 和日志文本阈值。
- 保持聊天范围的兼容导出，并明确 lorebook/worldInfo 命名边界。

验证情况：

- 全量回归通过：43 个 JavaScript 文件语法检查，29 个无参数 `.mjs` 测试。

### 2026-07-11：完成 TC-42 质量脚本与测试入口工程化

变更类型：工程化 / 测试 / 文档

主要变化：

- 新增无依赖 `npm run check` 与 `npm test` 入口。
- 默认测试只执行 `*.test.mjs`，明确排除 websocket 运行时辅助脚本。

验证情况：

- `npm run check` 通过 43 个 JS 文件；`npm test` 通过 29 个单元测试。

### 2026-07-11：对齐 v0.7.0 manifest 版本

变更类型：版本 / 文档

主要变化：

- 扩展和原生安装副本的 manifest 版本同步为 `0.7.0`。
- 未创建 tag，未推送远程仓库。

验证情况：

- 两份 manifest 均为 `0.7.0`；`npm run check` 通过 43 个 JS 文件，`npm test` 通过 29 个单元测试。
